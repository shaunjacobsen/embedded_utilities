...
const readSensorData = () => {
  sensor
    ...
    .then((data) => {
      ...
      let formattedTime = time.format('YYYY-MM-DD_HH:mm:ss.SSS');
      //console.log(`data = ${JSON.stringify(data, null, 2)}`);
      console.log('Temperature:', temp, 'Humidity:', humidity);
      const dataString = `${formattedTime}\tTemp: ${temp} C\tHumidity: ${humidity}%\n`;
      fs.writeFile(`data/${formattedDate}.txt`, dataString);

      let stream = fs.createWriteStream(`data/${formattedDate}.txt`, {flags:'a'});
      stream.write(dataString);
      stream.end();

      //matrix.writeArray(numbers.textRepresentationOfInteger(temp))
      setTimeout(readSensorData, 60000);
    })
    .catch((err) => {
